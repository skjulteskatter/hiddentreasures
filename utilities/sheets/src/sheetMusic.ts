import { GetMXMLData, http, stApi } from './http';
...
import { OpenSheetMusicDisplay, TransposeCalculator, MXLHelper } from 'opensheetmusicdisplay';
...
export async function getSheetMusicDetails(id: string): Promise<SheetMusicDetails | null> {
    ...
    const instruments: string[]= instance.Sheet.Instruments.map(({ NameLabel }) => NameLabel.text)
...
export async function getSheetMusicImages(options: ISheetMusicOptions) : Promise<string[]> {
    ...
    try {
        ...
        osmdInstance.TransposeCalculator = osmdInstance.TransposeCalculator ?? new TransposeCalculator();

        osmdInstance.Sheet.Transpose = options.clef === "bass" ? options.transposition - 12 : options.transposition;

        var changedInstruments: boolean = includeInstruments(options, osmdInstance);

        switch (options.guitarChords){
            case "between":
                osmdInstance.EngravingRules.MinSkyBottomDistBetweenSystems = DEFAULT_CHORD_HEIGHT * scale;
                osmdInstance.EngravingRules.PageTopMargin = DEFAULT_CHORD_HEIGHT * scale;
            break;
            case "top": 
                osmdInstance.EngravingRules.MinSkyBottomDistBetweenSystems = DEFAULT_MinSkyBottomDistBetweenSystems;
                osmdInstance.EngravingRules.PageTopMargin = DEFAULT_CHORD_HEIGHT * scale * Math.ceil(getNumberOfUniqueChords(await getOrSetFile(options.id)) / _numberOfGuitarChordsPerLine);
            break;
            case "none":
                osmdInstance.EngravingRules.MinSkyBottomDistBetweenSystems = DEFAULT_MinSkyBottomDistBetweenSystems;
                osmdInstance.EngravingRules.PageTopMargin = DEFAULT_PageTopMargin;
            break;
        }

        (options.clef === "treble" && options.transposition === 0 && !changedInstruments) || osmdInstance.updateGraphic();
    ...
    for (let pageNumber : number = 1; true; pageNumber++) {
        ...
        switch(options.guitarChords){
            case "top": 
                markupString = await GetTopChordSVG(markupString, scale, _numberOfGuitarChordsPerLine);
            break;
            case "between":
        ...
        markupString = (options.optimizeSVGsize ? OptimizeSVG(markupString) : markupString.trimStart());
...
function getNumberOfUniqueChords(musicXml: string) : number{
    return getAllUniqueChords(musicXml).length;
}

function getAllUniqueChords(musicXml: string): string[] {
    let differentChords : string[] = [];
    let harmonyArray : RegExpMatchArray | null = musicXml.match(/(?<=<harmony)(.*?)(<\/harmony>)/g);
    ...
    for (let i = 0; i < harmonyArray.length; i++) {
        ...
        const innerHarmonyArray : RegExpMatchArray | null = harmonyArray[i].toString().replace(/\s/g, "").match(/(?<=>)(.*?)(?=<\/harmony>)/g)
...
async function getOrSetFile(id: string) : Promise<string> {
    if (dataFiles[id]) {
        return dataFiles[id]
    }

    const sourceUrl = JSON.parse(await stApi.get('/api/Utilities/File/' + id + "/ContentUrl"));
...
async function getOrSetOsmdInstance(options: ISheetMusicOptions) : Promise<OpenSheetMusicDisplay | null> {
...
async function createOsmdInstance(options: ISheetMusicOptions): Promise<OpenSheetMusicDisplay | null> {
    ...
    div.setAttribute("offsetWidth", options.width);
    ...
    Object.defineProperties(window.HTMLElement.prototype, {
        ...
        offsetWidth: {
            get: function () { return options.width; }
        }
    });
...
function includeInstruments(options: ISheetMusicOptions, osmdInstance: OpenSheetMusicDisplay) : boolean {
    ...
    for (let i = 0; i < osmdInstance.Sheet.Instruments.length; i++) {
        ...
        if (options.instruments){
            if (!containsCaseInsensitive(options.instruments, instrument.NameLabel.text) && options.instruments.length > 0 && osmdInstance.Sheet.Instruments.length > 1) {
