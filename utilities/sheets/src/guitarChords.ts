...
function ExpandJSONChord(compressedChord: any): JSON {
    return JSON.parse(
        JSON.stringify(compressedChord)
            .replace(/"f"/g, '"fingers"')
            .replace(/"b"/g, '"barres"')
...
export async function getGuitarChord(
    options: IGuitarChordOptions
): Promise<Object[]> {
    let chordsArray: any[] = []
    ...
    try {
        ...
        let numberOfChordsLimit = options.allCombinations
        ...
        for (let i = 0; i < chordData.length; i++) {
            ...
            chordsArray.push(
                options.imageFormat === "json"
                    ? ExpandJSONChord(chordData[i])
                    : GetChordSVGImage(chordData[i], options.width)
            )
        }
    } catch (e) {
...
function ResizeSVG(svgData: string, width: number): string {
    ...
    return svgData.replace(
        ...
        ` width="${width}" height="${
            width * scale
        }" viewBox="${posX} ${posY} ${oldWidth} ${oldHeight}" `
    )
}
...
async function GetJSONFriendlyChordName(chordName: string): Promise<string> {
    ...
    const addChords: RegExpMatchArray | null =
        chordName.match(/(?<=\()\d(?=\))/g)
    if (addChords) {
    ...
    if (subsituteKeys.has(startOfString)) {
        chordName = chordName.replace(
            startOfString,
            subsituteKeys.get(startOfString) || startOfString
        )
    }
    ...
    if (subsituteKeys.has(endOfString)) {
        chordName = chordName.replace(
            endOfString,
            subsituteKeys.get(endOfString) || endOfString
        )
    }

    // --- This might be overdoing trying to find the correct chord. Might be better to just ignore parantheses ---
    const parantheseChords: RegExpMatchArray | null =
        chordName.match(/(?<=\().*?(?=\))/g)
    if (parantheseChords) {
        while (true) {
...
export function CouldBeChord(s: string): boolean {
    return !!s.match(
        /^[A-G](dim|maj|m|sus|aug|add|b|#|o|ø|[0-9]|\(|\)|alt )*((\/[A-G](#|b)?)|())$/
    )
}
