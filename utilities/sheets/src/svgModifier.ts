...
import { CouldBeChord, getGuitarChord as GetGuitarChord, IsChord } from './guitarChords';
...
async function GetUniqueChordsFromSVG(svg: string ): Promise<string[]>{
    ...
    for (let i = 0; i < textElements.length; i++) {
        ...
        if (!alreadyAcceptedChords.includes(textContent) && (CouldBeChord(textContent) && (await IsChord(textContent)))){
 ...
 */
export async function GetTopChordSVG(svg:string, scale: number, numberOfGuitarChordsPerLine: number) : Promise<string>{

    const startStringArray : RegExpMatchArray | null = svg.match(/<svg(.*?)<\/rect>/);
    if (!startStringArray){
        return svg;
    }
    ...
    const distanceBetweenChords : number = widthOfSVG / (numberOfGuitarChordsPerLine);
    ...
    for (let i = 0; i < chordTextElements.length; i++) {
        ...
        chordWidthHeight = chordWidthHeight[0] == 0 ? GetElementWidthHeight(chord) : chordWidthHeight;

        let y = Math.floor(i/numberOfGuitarChordsPerLine);
        let x = i % numberOfGuitarChordsPerLine;

        let embeddedGuitarChord : string = ` <g>${chord.replace(`<svg `, `<svg x='${(x * distanceBetweenChords) + ((distanceBetweenChords - chordWidthHeight[0]) / 2)}' y='${y * chordWidthHeight[1]}' `)}</g> `;
 ...
 */
export async function GetBetweenChordSVG(svg:string , scale: number) : Promise<string>{
    ...
    for (let i = 0; i < chordTextElements.length; i++) {
        ...
        let nextChord : string = chordTextElements[i+1 >= chordTextElements.length ? 0 : i+1];
        let nextPosition = GetTextElementXYPosition(nextChord);

        nextPosition[0] += (DEFAULT_WIDTH_OF_ARIAL_CHAR*GetTextTagContent(nextChord).length);

        let xDifference: number = Math.abs(position[0] - nextPosition[0])

        if (xDifference < DEFAULT_CHORD_WIDTH * scale && Math.abs(position[1] - nextPosition[1]) < 5 * scale){
        ...
        if (!chordSVGs.has(chordName + chordWidth)){
            ...
            const chord: string = await GetGuitarChord(guitarChordOptions) + '';
            if (chord != ''){
        ...
        if (chordSVGs.has(chordName + chordWidth)){
            ...
            let chordX = (position[0]-(chordDimensions[0]/2)) - (chordName.lastIndexOf('/'))
            let chordY = position[1]-(chordDimensions[1]) - (SPACE_BETWEEN_MEASURE_AND_CHORD*scale)

            let embeddedGuitarChord : string = ` <g>${chord.replace(`<svg `, `<svg x='${chordX}' y='${chordY}' `)}</g> `
            modifiedSVG = modifiedSVG.replace(textElement, ` ${embeddedGuitarChord} ${textElement}`)
        }
